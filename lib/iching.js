...
export function castIChing(){
 ...
 const primary=numFor(lines),relating=numFor(changed);
 const moving=lines.map((v,i)=>(v===6||v===9)?i+1:null).filter(Boolean);
 return {method:'three-coin',lineOrder:'bottom-up',lines,primary,primaryName:names[primary],relating,relatingName:names[relating],moving,createdAt:new Date().toISOString()};
}
export const lineGlyph=v=>(v===7||v===9)?'━━━━━━':'━━  ━━';
